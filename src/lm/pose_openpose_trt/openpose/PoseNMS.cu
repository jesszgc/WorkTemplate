...
namespace op
{
	...
	template <typename T>
	__global__ void nmsRegisterKernelBatch(
		int* kernelPtr, const T* const sourcePtr, const int w, const int h, const int kernelSize,const int sourceSize ,const int batchsize,const T threshold)
	{
		// get pixel location (x,y)
		const auto x = blockIdx.x * blockDim.x + threadIdx.x;
		const auto y = blockIdx.y * blockDim.y + threadIdx.y;
		...
		{
			auto* kernelPtrOffset = &kernelPtr[channelOffset+ kernelSize*i];
			const T* const sourcePtrOffset = &sourcePtr[channelOffset+ sourceSize*i];
            ...
            if (globalIdx != length - 1)
            {
                ...
                if (local[threadIdx.x] != local[threadIdx.x + 1])
                {
                    ...
                    if (peakIndex < maxPeaks) // limitation
                    {
                        ...
                        outputOffset[outputIndex + 2] = sourcePtrOffset[peakLocY*width + peakLocX] + 0.03; // no resize, score lower about 0.03 compare to caffe version.
                    }
                }
            }
    ...
    void nmsGpu(float* targetPtr, int* kernelPtr, const float* const sourcePtr, const float threshold,
                const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize, const Point<float>& offset)
    {
        try
        {
            ...
            // time = 2.71 ms
			thrust::device_ptr<int> kernelThrustPtr = thrust::device_pointer_cast(kernelPtr);
		...
		const std::array<int, 4>& sourceSize, const Point<float>& offset, const int batchsize, const int peaksSize, const int kernelSize, const int resizemapSize)
	{
		...
		const dim3 numBlocksRegister{ getNumberCudaBlocks(width, threadsPerBlockRegister.x),
			getNumberCudaBlocks(height, threadsPerBlockRegister.y),
			getNumberCudaBlocks(channels, threadsPerBlockRegister.z) };
		nmsRegisterKernelBatch << <numBlocksRegister, threadsPerBlockRegister >> > (kernelPtr, sourcePtr, width, height, kernelSize, resizemapSize,batchsize, threshold);
		...
		const dim3 threadsPerBlockWrite{ THREADS_PER_BLOCK, 1 };
		const dim3 numBlocksWrite{ getNumberCudaBlocks(imageOffset, threadsPerBlockWrite.x),
			getNumberCudaBlocks(channels, threadsPerBlockWrite.z) };
		for (int i = 0; i < batchsize; i++)
		{
			thrust::device_ptr<int> kernelThrustPtr = thrust::device_pointer_cast(kernelPtr+i*kernelSize);
			thrust::exclusive_scan(kernelThrustPtr, kernelThrustPtr + channels*imageOffset, kernelThrustPtr);

			writeResultKernel << <numBlocksWrite, threadsPerBlockWrite >> > (
				targetPtr+i*peaksSize, imageOffset, kernelPtr+i*kernelSize, sourcePtr+i*resizemapSize, width, height,
				maxPeaks, offset.x, offset.y, offsetTarget);
		}
