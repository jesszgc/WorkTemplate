...
namespace op
{
	...
	template <typename T>
	__global__ void resizeKernelBatch(
		T* targetPtr, const T* const sourcePtr, const int widthSource, const int heightSource, const int widthTarget,
		const int heightTarget, const int channels,const int batchsize)
	{
		const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
		const auto y = (blockIdx.y * blockDim.y) + threadIdx.y;
		const auto channel = (blockIdx.z * blockDim.z) + threadIdx.z;
		if (x < widthTarget && y < heightTarget)
		{
			const auto sourceArea = widthSource * heightSource;
			const auto targetArea = widthTarget * heightTarget;
			const auto sourceVolume = widthSource * heightSource*channels;
			const auto targetVolume = widthTarget * heightTarget*channels;
			const T xSource = (x + T(0.5f)) * widthSource / T(widthTarget) - T(0.5f);
			const T ySource = (y + T(0.5f)) * heightSource / T(heightTarget) - T(0.5f);
			for (int i = 0; i < batchsize; i++)
			{
				const T* const sourcePtrChannel = sourcePtr + i*sourceVolume + channel * sourceArea;
				targetPtr[i*targetVolume+channel * targetArea + y*widthTarget + x] = bicubicInterpolate(
					sourcePtrChannel, xSource, ySource, widthSource, heightSource, widthSource);
			}
		}
		...
		int widthTarget, int heightTarget, int batchsize)
	{
		...
		const dim3 numBlocks{
			getNumberCudaBlocks(widthTarget, threadsPerBlock.x),
			getNumberCudaBlocks(heightTarget, threadsPerBlock.y),
			getNumberCudaBlocks(channelSource, threadsPerBlock.z) };
		resizeKernel << <numBlocks, threadsPerBlock >> > ((float*)targetGpuPtr, (float*)sourcePtrs, widthSouce, heightSource, widthTarget, heightTarget);
	}
		...
		int widthTarget, int heightTarget, int batchsize)
	{
		...
		const dim3 numBlocks{
			...
			getNumberCudaBlocks(heightTarget, threadsPerBlock.y),
			getNumberCudaBlocks(channelSource, threadsPerBlock.z) };
		resizeKernelBatch << <numBlocks, threadsPerBlock >> > ((float*)targetGpuPtr, (float*)sourcePtrs, widthSouce, heightSource, widthTarget, heightTarget, channelSource, batchsize);
	}
		...
		T* targetPtr, const unsigned char* const srcPtr, const int width, const int height, const int channels)
	{
		...
		if (x < width && y < height)
		{
			...
			targetPtr[targetIndex] = T(((float)(srcPtr[srcIndex]) - 127.5f) / 256.f);
		}
	}
	...
	__global__ void resizeGPU(const unsigned char*src, int srcWidth, int srcHeight, unsigned char *dst, int dstWidth, int dstHeight)
	{
		double srcXf;
		double srcYf;
		int srcX;
		...
		srcXf = x* ((float)srcWidth / dstWidth);//这个float重要阿，不然会算是精度的，尼玛
		...
		//B chanel
		dstOffset = (y*dstWidth + x) * 3;
		dst[dstOffset] = 0;
		dst[dstOffset] += (1 - u)*(1 - v)*src[(srcY*srcWidth + srcX) * 3];
		dst[dstOffset] += (1 - u)*v*src[((srcY + 1)*srcWidth + srcX) * 3];
		dst[dstOffset] += u*(1 - v)*src[(srcY*srcWidth + srcX + 1) * 3];
		dst[dstOffset] += u*v*src[((srcY + 1)*srcWidth + srcX + 1) * 3];

		//G chanel
		dstOffset = (y*dstWidth + x) * 3 + 1;
		dst[dstOffset] = 0;
		dst[dstOffset] += (1 - u)*(1 - v)*src[(srcY*srcWidth + srcX) * 3 + 1];
		dst[dstOffset] += (1 - u)*v*src[((srcY + 1)*srcWidth + srcX) * 3 + 1];
		dst[dstOffset] += u*(1 - v)*src[(srcY*srcWidth + srcX + 1) * 3 + 1];
		dst[dstOffset] += u*v*src[((srcY + 1)*srcWidth + srcX + 1) * 3 + 1];

		//R chanel
		dstOffset = (y*dstWidth + x) * 3 + 2;
		dst[dstOffset] = 0;
		dst[dstOffset] += (1 - u)*(1 - v)*src[(srcY*srcWidth + srcX) * 3 + 2];
		dst[dstOffset] += (1 - u)*v*src[((srcY + 1)*srcWidth + srcX) * 3 + 2];
		dst[dstOffset] += u*(1 - v)*src[(srcY*srcWidth + srcX + 1) * 3 + 2];
		dst[dstOffset] += u*v*src[((srcY + 1)*srcWidth + srcX + 1) * 3 + 2];
	...
	{
		int uint = 16;//测了很多次，发现这个比例是最佳的
		dim3 grid((dstWidth + uint - 1) / uint, (dstHeight + uint - 1) / uint);
		dim3 block(uint, uint);
		resizeGPU << <grid, block >> > (src, srcWidth, srcHeight, dst, dstWidth, dstHeight);
	}

	__global__ void resizeAndChangeGPU(const unsigned char*src, int srcWidth, int srcHeight, float *dst, int dstWidth, int dstHeight, float fMean, float fScale)
	{
		double srcXf;
		double srcYf;
		int srcX;
		...
		dst[dstOffset] += u*(1 - v)*src[(srcY*srcWidth + srcX + 1) * 3];
		dst[dstOffset] += u*v*src[((srcY + 1)*srcWidth + srcX + 1) * 3];
		dst[dstOffset] = (dst[dstOffset] - fMean)*fScale;
	...
	void resize8UC332FC3Gpu(const unsigned char*src, int srcWidth, int srcHeight, float *dst, int dstWidth, int dstHeight, float fMean, float fScale)
	{
		...
		dim3 grid((dstWidth + uint - 1) / uint, (dstHeight + uint - 1) / uint);
		dim3 block(uint, uint);
		resizeAndChangeGPU << <grid, block >> > (src, srcWidth, srcHeight, dst, dstWidth, dstHeight,fMean,fScale);
	}
	...
	void resize8UC332FC3Gpu(const unsigned char*src, int srcWidth, int srcHeight, float *dst, int dstWidth, int dstHeight, float fMean, float fScale,cudaStream_t stream)
	{
		...
		dim3 grid((dstWidth + uint - 1) / uint, (dstHeight + uint - 1) / uint);
		dim3 block(uint, uint);
		resizeAndChangeGPU << <grid, block,0,stream >> > (src, srcWidth, srcHeight, dst, dstWidth, dstHeight, fMean, fScale);
	}
