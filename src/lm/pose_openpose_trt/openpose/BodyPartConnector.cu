...
namespace op
{
	...
	__global__ void pafScoreKernel(
		...
		const unsigned int* const bodyPartPairsPtr,		const unsigned int* const mapIdxPtr, const unsigned int maxPeaks, const int numberBodyPartPairs,
		...
		const T defaultNmsThreshold)
	{
		...
		if (peakA < maxPeaks && peakB < maxPeaks)// if (pairIndex < numberBodyPartPairs && peakA < maxPeaks && peakB < maxPeaks)			
		{
	...
	template <typename T>
	__global__ void pafScoreKernelBatch(
		T* pairScoresPtr, int nPairScoreSize, const T* const heatMapPtr, int nHeatMapSize, const T* const peaksPtr, int nPeaksSize, int batchsize,
		const unsigned int* const bodyPartPairsPtr, const unsigned int* const mapIdxPtr, const unsigned int maxPeaks, const int numberBodyPartPairs,
		const int heatmapWidth, const int heatmapHeight, const T interThreshold, const T interMinAboveThreshold,
		const T defaultNmsThreshold)
	{
		...
		if (peakA < maxPeaks && peakB < maxPeaks)// if (pairIndex < numberBodyPartPairs && peakA < maxPeaks && peakB < maxPeaks)			
		{
			...
			{
				const T numberPeaksB = peaksPtr[3 * partB*(maxPeaks + 1)+i*nPeaksSize];
				const T numberPeaksA = peaksPtr[3 * partA*(maxPeaks + 1)+i*nPeaksSize];
				...
				if (peakA < numberPeaksA && peakB < numberPeaksB)
				{
					...
					const T* const bodyPartA = peaksPtr + i*nPeaksSize + (3 * (partA*(maxPeaks + 1) + peakA + 1));
					const T* const bodyPartB = peaksPtr + i*nPeaksSize + (3 * (partB*(maxPeaks + 1) + peakB + 1));
					const T* const mapX = heatMapPtr+i*nHeatMapSize + mapIdxX*heatmapWidth*heatmapHeight;
					const T* const mapY = heatMapPtr+i*nHeatMapSize + mapIdxY*heatmapWidth*heatmapHeight;
					pairScoresPtr[outputIndex+i*nPairScoreSize] = process(
						bodyPartA, bodyPartB, mapX, mapY, heatmapWidth, heatmapHeight, interThreshold,
						interMinAboveThreshold, defaultNmsThreshold);
				}
				else
				{
					pairScoresPtr[outputIndex+i*nPairScoreSize] = -1;
				}
			}
		...
		const float* const peaksGpuPtr)
	{
			...
			const auto bodyPartPairs = std::vector<unsigned int>{ 1,8,   1,2,   1,5,   2,3,   3,4,   5,6,   6,7,   8,9,   9,10,  10,11, 8,12,  12,13, 13,14,  1,0,   0,15, 15,17,  0,16, 16,18,   2,17,  5,18,   14,19,19,20,14,21, 11,22,22,23,11,24 };

			const auto numberBodyParts = 25;// getPoseNumberBodyParts(poseModel);
			unsigned int numberBodyPartPairs = (unsigned int)(bodyPartPairs.size() / 2);
			const auto totalComputations = pairScoresCpuDims[0]* pairScoresCpuDims[1]* pairScoresCpuDims[2];
			...
			cudaMemcpy(pairScoresCpu, pairScoresGpuPtr, totalComputations * sizeof(float),
			...
			const auto pairConnections = pafPtrIntoVector(
				pairScoresCpu, pairScoresCpuDims,peaksPtr, maxPeaks, bodyPartPairs, numberBodyPartPairs);
	...
	template <typename T>
	void removePeopleBelowThresholdsAndFillFacesLPF(
		...
		// const int minSubsetCnt, const T minSubsetScore, const int maxPeaks, const bool maximizePositives)
	{
		...
		{
			removePeopleBelowThresholdsAndFillFacesLPF(
				validSubsetIndexes, numberPeople, peopleVector, numberBodyParts, minSubsetCnt, minSubsetScore,
				true, peaksPtr);
		}
	...
	template <typename T>
	void peopleVectorToPeopleArrayLPF(
		std::vector< std::vector< PosePt > > & outData, const T scaleFactorX, const T scaleFactorY,
		...
		const T* const peaksPtr, const int numberPeople, const unsigned int numberBodyParts)
	{
		...
		if (numberPeople > 0)
		{
			...
			PosePt ppt;
			ppt.fx = 0.f; ppt.fy = 0.f; ppt.fs = 0.f;
			std::vector< PosePt > vecPose;
			vecPose.resize(numberBodyParts, ppt);
			outData.resize(numberPeople, vecPose);
			...
			for (auto person = 0u; person < validSubsetIndexes.size(); person++)
			{
				...
				for (auto bodyPart = 0u; bodyPart < numberBodyParts; bodyPart++)
				{
					...
					if (bodyPartIndex > 0)
					{
						outData[person][bodyPart].fx = peaksPtr[bodyPartIndex - 2] * scaleFactorX;
						outData[person][bodyPart].fy = peaksPtr[bodyPartIndex - 1] * scaleFactorY;
						outData[person][bodyPart].fs = peaksPtr[bodyPartIndex];
					}
				}
			}
		}	
		...
		const float* const peaksGpuPtr)
	{
		...
		removePeopleBelowThresholdsAndFillFacesLPF(
			...
			maximizePositives, peaksPtr);
		// Fill and return poseKeypoints
		peopleVectorToPeopleArrayLPF(outData, scaleFactorX,scaleFactorY, peopleVector, validSubsetIndexes, peaksPtr, numberPeople,numberBodyParts);

	}

	void connectBodyPartsGpuLPF_Part1( const float* const heatMapGpuPtr, const int resizeMapSize,
		const Point<int>& heatMapSize, const int maxPeaks, const float interMinAboveThreshold,
		const float interThreshold, const float defaultNmsThreshold,
		...
		const float* const peaksGpuPtr, const int peakSize, const int batchsize)
	{
		...
		const dim3 numBlocks{
			...
			getNumberCudaBlocks(maxPeaks, THREADS_PER_BLOCK.y),
			getNumberCudaBlocks(numberBodyPartPairs, THREADS_PER_BLOCK.z) };
		pafScoreKernelBatch << <numBlocks, THREADS_PER_BLOCK >> >(pairScoresGpuPtr, pairScoresSize,heatMapGpuPtr, resizeMapSize, peaksGpuPtr, peakSize, batchsize,
			bodyPartPairsGpuPtr, mapIdxGpuPtr, maxPeaks, (int)numberBodyPartPairs, heatMapSize.x, heatMapSize.y, interThreshold,
			interMinAboveThreshold, defaultNmsThreshold);
	}
        ...
        const T* const mapY, const Point<int>& heatMapSize, const T interThreshold, const T interMinAboveThreshold)
    {
        ...
        // std::cout << "vectorAToBX: " << vectorAToBX << " vectorAToBY: " << vectorAToBY << " vectorAToBMax: " << vectorAToBMax << std::endl;
        // If the peaksPtr are coincident. Don't connect them.
        if (vectorNorm > 1e-6)
        {
            ...
            for (auto lm = 0; lm < numberPointsInLine; lm++)
            {
                ...
                const auto score = (vectorAToBNormX*mapX[idx] + vectorAToBNormY*mapY[idx]);
                // std::cout << "mX: " << mX << " mY: " << mY << " lm*vectorAToBXInLine: " << lm*vectorAToBXInLine << std::endl;
                // std::cout << "vectorAToBNormX: " << vectorAToBNormX << " maxX[idx]: " << mapX[idx] << " vectorAToBNormY: " << vectorAToBNormY << " mapY: " << mapY[idx] << std::endl; 
                // std::cout << "mX: " << mX << " mY: " << mY << " idx: " << idx << " score: " << score << std::endl;
                if (score > interThreshold)
                {
                    sum += score;
                    count++;
            ...
            // std::cout << "count/T(numberPointsInLine): " << count << "/" << numberPointsInLine << "=" << count/T(numberPointsInLine) << std::endl;
            if (count/T(numberPointsInLine) > interMinAboveThreshold){
                return sum/count;
            }
        }
    ...
    std::vector<std::pair<std::vector<int>, T>> createPeopleVector(
        ...
        const std::vector<unsigned int>& bodyPartPairs, const unsigned int numberBodyParts/*25*/,
        const unsigned int numberBodyPartPairs/*26*/, const Array<T>& pairScores)
    {
        // std::vector<std::pair<std::vector<int>, double>> refers to:
        //     - std::vector<int>: [body parts locations, #body parts found]
        //     - double: person subset score
        ...
        for (auto pairIndex = 0u; pairIndex < numberBodyPartPairs; pairIndex++)
        {
            ...
            else // if (numberPeaksA != 0 && numberPeaksB != 0)
            {
                ...
                if (heatMapPtr != nullptr)
                {
                    ...
                    for (auto i = 1; i <= numberPeaksA; i++)
                    {
                        ...
                        for (auto j = 1; j <= numberPeaksB; j++)
                        {
                            ...
                            // std::cout << "score of " << i << " and " << j << " : " << scoreAB << std::endl;
                            if (scoreAB > 1e-6)
                ...
                if (!abConnections.empty())
                {
                    ...
                    else if ( pairIndex==18 || pairIndex==19 )
                    {
                        for (const auto& abConnection : abConnections)
                        {
                            const auto indexA = std::get<0>(abConnection);
        ...
        const std::vector<unsigned int>& bodyPartPairs, const unsigned int numberBodyPartPairs)
    {
        try
        {
            ...
            for (auto pairIndex = 0u; pairIndex < numberBodyPartPairs; pairIndex++)
            {
                ...
                const auto numberPeaksA = positiveIntRound(candidateAPtr[0]);
                const auto numberPeaksB = positiveIntRound(candidateBPtr[0]);
                const auto firstIndex = (int)pairIndex*pairScoresDims[1]*pairScoresDims[2];
                // E.g., neck-nose connection. For each neck
                for (auto indexA = 0; indexA < numberPeaksA; indexA++)
                {
                    const auto iIndex = firstIndex + indexA*pairScoresDims[2];
                    // E.g., neck-nose connection. For each nose
