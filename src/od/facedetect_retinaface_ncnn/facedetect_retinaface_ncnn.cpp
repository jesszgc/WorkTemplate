...
#include <vector>
#include<iostream>
#include "facedetect_retinaface_ncnn.h"
...
faceDetectretinaNcnn::faceDetectretinaNcnn(std::string modelpath,int gpu,int threadnum,int modelsize,int maxbatchsize)
{
    ncnn::Net *retinaface=new ncnn::Net() ;
    ...
    int ret1= retinaface->load_param(parampath.c_str());//"mnet.25-opt.param");
    int ret2= retinaface->load_model(binpath.c_str());//"mnet.25-opt.bin");
    if(ret1<0||ret2<0){
        return;
    }
    net=retinaface;
...
faceDetectretinaNcnn::~faceDetectretinaNcnn() {
...
int faceDetectretinaNcnn::Detect(akdData *data,std::vector<std::vector<objBox>> &res)
{
    ncnn::Net* retinaface=(ncnn::Net*)net;

    for(int i=0;i<data->num;++i){
        int img_w = data->widths[i];//.cols;
        int img_h = data->heights[i];//bgr.rows;

        ncnn::Mat in = ncnn::Mat::from_pixels((const unsigned char*)(data->ptr[i]), ncnn::Mat::PIXEL_BGR2RGB, img_w, img_h);
...
odService* facedetectinit_retinaface_ncnn(odinitconfig config)
{
    faceDetectretinaNcnn * ch=new faceDetectretinaNcnn(config.modelpath,config.gpu,config.threadnum,config.maxnetsize,config.maxbatchsize);
    if(ch->initsuccess<0)
    {
        return nullptr;
    }
