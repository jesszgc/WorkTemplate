...
#include <vector>
#include<iostream>
#include "bodydetect_yolov5_trt.h"
#include "yolov5.h"
...
bodyDetectYolov5Trt::bodyDetectYolov5Trt(std::string modelpath,int gpu,int threadnum,int modelsize,int maxbatchsize)
{
...
bodyDetectYolov5Trt::~bodyDetectYolov5Trt() {
...
int bodyDetectYolov5Trt::Detect(akdData *data,std::vector<std::vector<objBox>> &res)
{
...
odService* bodydetectinit_yolov5_trt(odinitconfig config)
{
    bodyDetectYolov5Trt * ch=new bodyDetectYolov5Trt(config.modelpath,config.gpu,config.threadnum,config.maxnetsize,config.maxbatchsize);
    if(ch->initsuccess<0)
    {
        return nullptr;
    }
